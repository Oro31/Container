		...
		public:
			...
			template<class InputIterator>
				vector(InputIterator first, InputIterator last,
					const allocator_type &alloc = allocator_type(),
					typename ft::enable_if<!ft::is_integral<InputIterator>::value>::type * = 0) : current_end(NULL) {
				the_allocator = alloc;
				assign(first, last);
			};
			//
			...
			void		resize(size_type sz, T c = T()) {
				if (sz <= size()) {
					for (size_type i = size(); i > sz; --i) {
						--current_end;
						the_allocator.destroy(current_end.base());
					}
				} else {
					if (sz > capacity()) {
						reserve(sz);
			...
			//
			size_type capacity() const {if (!buffer_start.base()) {return 0;} return end_of_buffer - buffer_start;}
			//
			...
			void	push_back(const T &x) {
				reserve(size() + 1);
				the_allocator.construct(current_end.base(), x);
				++current_end;
			};
			//
			void	pop_back() {
			...
			iterator	insert(iterator position, const T &x) {
				...
				if (tmp < current_end)
					the_allocator.destroy(tmp.base());
				...
				for (iterator it = ++tmp; it != current_end; ++it) {
					...
					the_allocator.construct(it.base(), *tpmp);
					++tpmp;
				}
				return buffer_start + pos;
			};
			...
			iterator	insert(iterator position, size_type count, const T &x) {
				...
				while (tmp != end_of_buffer && tpmp != p.current_end) {
					if (tmp < current_end)
						the_allocator.destroy(tmp.base());
					the_allocator.construct(tmp.base(), *tpmp);
					++tmp;
					++tpmp;
				}
				current_end += count;
			...
			iterator	erase(iterator position) {
				...
				for (iterator it = position; it != current_end; ++it) {
					the_allocator.destroy(it.base());
					the_allocator.construct(it.base(), *(it + 1));
				}
				the_allocator.destroy(current_end.base());
				return position;
			};
			...
			iterator	erase(iterator first, iterator last) {
				...
				for (iterator it = first; it != current_end; ++it) {
					the_allocator.destroy(it.base());
				}

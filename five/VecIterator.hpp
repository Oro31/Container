...
namespace ft {
	...
	class VecIterator {
		...
		public:
			...
			VecIterator &operator=(const VecIterator &mit) {p = mit.base(); return *this;};
			VecIterator(const VecIterator<typename std::remove_const<T>::type> &mit) : p(mit.base()) {};
//			VecIterator(const VecIterator<const value_type> &mit) {*this = mit.base();};
			...
			VecIterator &operator+=(difference_type n) {for (int i = 0; i < n; i++) {++p;} return *this;};
			VecIterator &operator-=(difference_type n) {for (int i = 0; i < n; i++) {--p;} return *this;};
	...
	template<class Iterator>
		Iterator operator+(const typename Iterator::difference_type &n, const Iterator &it) {Iterator r(it); r+=n; return r;};
